...
if __name__ == "__main__":
    ...
    # 复制 label_transform_info.json 到输出目录
    try:
        # 构建源文件路径（来自 1_split 目录）
        split_dir = args.input_dir  # 应该是 data/outputs/1_split
        ...
        if os.path.exists(source_label_file):
            ...
            if os.path.exists(target_label_file):
                # print(f"[PromptGen] ℹ️  Info: label_transform_info.json already exists in target directory, skipping copy: {target_label_file}")
                pass
            else:
                # 复制文件
                shutil.copy2(source_label_file, target_label_file)
                print(f"[PromptGen] SUCCESS: Copied label_transform_info.json to: {target_label_file}")
        else:
            print(f"[PromptGen] WARNING:  Warning: Source label_transform_info.json not found: {source_label_file}")
            
    except Exception as e:
        print(f"[PromptGen] WARNING:  Warning: Failed to copy label_transform_info.json: {e}")
